...
const WORDS = [
  ...
  { spanish: 'casa',   english: 'house',  emoji: '🏠' },
  ...
  { spanish: 'sol',    english: 'sun',    emoji: '☀️' },
  { spanish: 'luna',   english: 'moon',   emoji: '🌙' },
];
...
function buildRound() {
  const picked = shuffle(WORDS).slice(0, TOTAL);
...
export default function SpanishVocabulary({ profileId, onClose }: Props) {
  ...
  const isCorrect = selected === q?.english;
      ...
      if (!saved && profileId) {
        ...
        if (user) {
          await supabase.from('spanish_results').insert({
            user_id: user.id, child_id: profileId, mode: 'vocabulary', score, total: TOTAL,
          });
        }
      }
    } else {
  ...
  if (finished) {
    ...
    const msg = pct === 1 ? '🏆 ¡Perfecto! Perfect score!' : pct >= 0.6 ? '🌟 ¡Muy bien! Great job!' : '💪 ¡Buena suerte! Keep practising!';
    return (
      <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
        <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} className="bg-white rounded-3xl shadow-2xl max-w-sm w-full text-center overflow-hidden">
          <div className="bg-gradient-to-r from-red-500 to-orange-500 px-6 py-4">
            <p className="text-white font-extrabold text-xl">🇪🇸 Vocabulary</p>
          </div>
          <div className="px-6 py-8">
            ...
            <p className="text-base font-bold text-gray-500 mb-6">{msg}</p>
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <motion.div key={index} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.25 }}
        ...
        <div className="bg-gradient-to-r from-red-500 to-orange-500 px-6 py-4 flex items-center justify-between">
          <div>
            <p className="text-white/80 text-xs font-semibold uppercase tracking-wide">🇪🇸 Vocabulary</p>
        ...
        <div className="px-6 py-8">
          <p className="text-center text-gray-400 text-sm mb-2">What does this mean in English?</p>
          <div className="text-center mb-6">
            <div className="text-6xl mb-2">{q.emoji}</div>
            <p className="text-4xl font-black text-gray-800">{q.spanish}</p>
          </div>
            ...
            {q.options.map((opt) => {
              ...
              return (
                <button key={opt} disabled={!!selected} onClick={() => { setSelected(opt); if (opt === q.english) setScore(s => s + 1); }}
                  className={`border-2 rounded-xl py-3 px-4 font-bold text-sm transition-colors ${bg}`}>
                  {opt}
                </button>
              );
            })}
          ...
          <AnimatePresence>
            {selected && (
              <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
                className={`rounded-xl px-4 py-3 text-center mb-4 ${isCorrect ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
                <p className={`font-extrabold text-sm ${isCorrect ? 'text-green-700' : 'text-red-700'}`}>
                  {isCorrect ? '✅ ¡Correcto! Well done!' : `❌ It's "${q.english}" — keep going!`}
                </p>
              </motion.div>
            )}
          </AnimatePresence>
